// CÓDIGO COMPLETO E CORRIGIDO DA TELA DE CADASTRO DE PRODUTO
...
// Esta classe de formatação de moeda que já tínhamos continua perfeita.
class CurrencyInputFormatter extends TextInputFormatter {
  final int maxDigitsBeforeDecimal;
  final int maxDigitsAfterDecimal;

  CurrencyInputFormatter(
      {this.maxDigitsBeforeDecimal = 7, this.maxDigitsAfterDecimal = 2});
...
class _TelaCadastroProdutoState extends State<TelaCadastroProduto> {
  ...
  final List<String> _tiposDeProduto = const [
    'ACESSÓRIO', 'ELETRICO', 'EPI', 'EXTINTOR', 'FERRAMENTA', 'HIDRANTE',
    'LOGISTICA', 'MANGUEIRA', 'MATERIAL ESCRITORIO', 'MATERIAL LIMPEZA', 'OBRA',
    'PRODUÇÃO', 'SINALIZAÇÃO', 'SISTEMA ALARME', 'SISTEMA BOMBA', 'UNIFORME'
  ];
  final List<String> _gruposDeProduto = const [
    'CONSUMO', "EPI'S", 'MATERIA PRIMA', 'REVENDA'
  ];
  // O "PORQUÊ": Adicionamos "PACOTE" para compatibilidade com dados antigos,
  // permitindo a edição sem erros, enquanto mantemos "PCT" como o padrão.
  final List<String> _unidadesDeMedida = const [
    'BD', 'BR', 'CX', 'KG', 'LATA', 'LTS', 'MTS', 'PACOTE', 'PAR', 'PCT', 'PÇ'
  ];
  ...
  void _salvar() async {
    if (_formKey.currentState!.validate() && !_isLoading) {
      ...
      try {
        ...
        final query =
        await db.collection('produtos').where('codigo', isEqualTo: codigo).get();
        if (query.docs.isNotEmpty) {
          if (!modoEdicao ||
              (modoEdicao && query.docs.first.id != widget.produtoParaEditar!.id)) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text('Erro: Este código de produto já existe.'),
        ...
        if (modoEdicao) {
          await db
              .collection('produtos')
              .doc(widget.produtoParaEditar!.id)
              .update(dadosProduto);
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            ...
            child: Column(
              ...
              children: [
                TextFormField(
                  ...
                  validator: (value) =>
                  (value == null || value.isEmpty) ? 'O código é obrigatório.' : null,
                  inputFormatters: [
                ...
                TextFormField(
                  ...
                  validator: (value) =>
                  (value == null || value.isEmpty) ? 'O nome é obrigatório.' : null,
                ),
                const SizedBox(height: 16),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _gruposDeProduto.map((String grupo) =>
                      DropdownMenuItem<String>(value: grupo, child: Text(grupo)))
                      .toList(),
                  onChanged: (String? novoValor) =>
                      setState(() => _grupoSelecionado = novoValor),
                  validator: (value) =>
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _unidadesDeMedida.map((String unidade) =>
                      DropdownMenuItem<String>(value: unidade, child: Text(unidade)))
                      .toList(),
                  onChanged: (String? novoValor) =>
                      setState(() => _unidadeSelecionada = novoValor),
                  validator: (value) =>
                ...
                TextFormField(
                  ...
                  inputFormatters: [
                    CurrencyInputFormatter(maxDigitsBeforeDecimal: 7),
                  ],
                  validator: (value) {
                ...
                SwitchListTile(
                  ...
                  subtitle: const Text(
                      'Produtos inativos não aparecerão na lista principal ou em novas movimentações.'),
                  value: _produtoAtivo,
                  ...
                  secondary: Icon(
                      _produtoAtivo ? Icons.check_circle : Icons.cancel,
